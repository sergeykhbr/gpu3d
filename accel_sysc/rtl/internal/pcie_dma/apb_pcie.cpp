...
void apb_pcie::generateVCD(sc_trace_file *i_vcd, sc_trace_file *o_vcd) {
    ...
    if (o_vcd) {
        ...
        sc_trace(o_vcd, r.req_cnt, pn + ".r_req_cnt");
        for (int i = 0; i < 16; i++) {
            char tstr[1024];
            RISCV_sprintf(tstr, sizeof(tstr), "%s.rx_req_data_arr%d", pn.c_str(), i);
            sc_trace(o_vcd, rx.req_data_arr[i], tstr);
        }
    }
...
void apb_pcie::comb() {
    ...
    if (wb_req_addr.read()(11, 2) == 0) {
        ...
        if (wb_req_addr.read()[2] == 0) {
            vb_rdata = rx.req_data_arr[wb_req_addr.read()(6, 3).to_int()](31, 0);
        } else {
            vb_rdata = rx.req_data_arr[wb_req_addr.read()(6, 3).to_int()](63, 32);
        }
    }
...
void apb_pcie::rxegisters() {
    for (int i = 0; i < 16; i++) {
        rx.req_data_arr[i] = vx.req_data_arr[i];
    }
}
