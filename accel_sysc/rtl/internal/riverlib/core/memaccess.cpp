...
void MemAccess::generateVCD(sc_trace_file *i_vcd, sc_trace_file *o_vcd) {
    ...
    if (o_vcd) {
        ...
        sc_trace(o_vcd, r.mmu_sv39, pn + ".r.mmu_sv39");
        sc_trace(o_vcd, r.mmu_sv48, pn + ".r.mmu_sv48");
        sc_trace(o_vcd, r.memop_type, pn + ".r.memop_type");
        sc_trace(o_vcd, r.memop_addr, pn + ".r.memop_addr");
        sc_trace(o_vcd, r.memop_wdata, pn + ".r.memop_wdata");
        sc_trace(o_vcd, r.memop_wstrb, pn + ".r.memop_wstrb");
        ...
        sc_trace(o_vcd, r.memop_res_pc, pn + ".r.memop_res_pc");
        sc_trace(o_vcd, r.memop_res_instr, pn + ".r.memop_res_instr");
        sc_trace(o_vcd, r.memop_res_addr, pn + ".r.memop_res_addr");
        sc_trace(o_vcd, r.memop_res_wtag, pn + ".r.memop_res_wtag");
        sc_trace(o_vcd, r.memop_res_data, pn + ".r.memop_res_data");
        sc_trace(o_vcd, r.memop_res_wena, pn + ".r.memop_res_wena");
        sc_trace(o_vcd, r.hold_rdata, pn + ".r.hold_rdata");
...
void MemAccess::comb() {
    ...
    if (i_flushd_valid.read() == 1) {
        vb_req_addr = i_flushd_addr.read();
    } else {
        vb_req_addr = i_memop_addr.read();
    }
    ...
    case 2:
        ...
        break;
    case 3:
        vb_memop_wdata = i_memop_wdata.read();
        vb_memop_wstrb = 0xFF;
        break;
    default:
        break;
    ...
    // Form Queue inputs:
    t_memop_debug = i_memop_debug.read();                   // looks like bug in systemc, cannot handle bool properly
    ...
    if ((r.memop_type.read()[MemopType_Store] == 0) || (r.memop_type.read()[MemopType_Release] != 0)) {
        ...
        vb_mem_rdata = r.memop_res_data.read();
    }
    switch (r.state.read()) {
    case State_Idle:
        v_queue_re = 1;
        if (queue_nempty.read() == 1) {
            ...
            if ((vb_res_addr.or_reduce() == 1)
                    && (((!vb_mem_type[MemopType_Store]) || vb_mem_type[MemopType_Release]) != 0)) {
                v.memop_res_wena = 1;
            } else {
                v.memop_res_wena = 0;
            }
    ...
    case State_WaitReqAccept:
        v_mem_valid = 1;
        v_mmu_ena = r.mmu_ena.read();
        v_mmu_sv39 = r.mmu_sv39.read();
        v_mmu_sv48 = r.mmu_sv48.read();
        vb_mem_type = r.memop_type.read();
