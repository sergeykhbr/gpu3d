...
void ICacheLru::generateVCD(sc_trace_file *i_vcd, sc_trace_file *o_vcd) {
    ...
    if (o_vcd) {
        ...
        sc_trace(o_vcd, r.req_addr_next, pn + ".r.req_addr_next");
        sc_trace(o_vcd, r.write_addr, pn + ".r.write_addr");
...
void ICacheLru::comb() {
        ...
        if (i_pmp_x.read() == 0) {
            ...
            if (i_pma_cached.read() == 1) {
                if (line_hit_o.read() == 0) {
                    ...
                    v.write_addr = r.req_addr_next.read();
    ...
    case State_WaitResp:
        if (i_mem_data_valid.read() == 1) {
            ...
            v.state = State_CheckResp;
            v.write_addr = r.req_addr.read();               // Swap addres for 1 clock to write line
            v.req_addr = r.write_addr.read();
    ...
    case State_CheckResp:
        v.req_addr = r.write_addr.read();                   // Restore req_addr after line write
        if ((r.req_mem_type.read()[REQ_MEM_TYPE_CACHED] == 0)
                || (r.load_fault.read() == 1)) {
            v_resp_valid = 1;
            vb_resp_data = vb_uncached_data;
            v_resp_er_load_fault = r.load_fault.read();
            if (i_resp_ready.read() == 1) {
                v.state = State_Idle;
            }
        } else {
